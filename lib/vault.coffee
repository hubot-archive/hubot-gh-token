...
class Vault
  ...
  getSecrets: ->
    unless process.env.HUBOT_GITHUB_TOKEN_FERNET_SECRETS?
      throw new Error("Please set a HUBOT_GITHUB_TOKEN_FERNET_SECRETS string in the environment")
    fernetSecrets = process.env.HUBOT_GITHUB_TOKEN_FERNET_SECRETS.split(",")
